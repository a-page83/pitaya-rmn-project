...
def filter(time_array, voltageAcc_array):
    t = time_array
...
def run_selected_functions():
    ...
    graph_name = entry_graph_name.get().strip()
    if not graph_name:
        messagebox.showwarning("Missing input", "Please enter a graph name.")
        return

    print(f"\nGraph name: {graph_name}")
    print("Executing selected functions:")
    ...
    if var4.get():
        filter(time_array=time_array,voltageAcc_array=voltageAcc_array)
    plt.show()



#############################################################""
# Initialize global file variable
selected_file = None

# Create main window
root = tk.Tk()
root.title("Graph Function Selector")
root.geometry("350x400")

# Entry for graph name
tk.Label(root, text="Enter graph name:").pack(pady=5)
entry_graph_name = tk.Entry(root, width=30)
entry_graph_name.pack(pady=5)

# File selection button
tk.Button(root, text="Choose File", command=choose_file).pack(pady=5)
label_file = tk.Label(root, text="No file selected", wraplength=300, fg="gray")
...
tk.Label(root, text="Select functions to execute:").pack(pady=5)
tk.Checkbutton(root, text="Function 1", variable=var1).pack(anchor='w', padx=30)
tk.Checkbutton(root, text="Function 2", variable=var2).pack(anchor='w', padx=30)
tk.Checkbutton(root, text="Function 3", variable=var3).pack(anchor='w', padx=30)
tk.Checkbutton(root, text="Function 4", variable=var4).pack(anchor='w', padx=30)

# Run button
tk.Button(root, text="Run", command=run_selected_functions).pack(pady=20)
