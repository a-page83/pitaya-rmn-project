...
sample_Amount               = 524288/4 #424*4              #Number of points measured (MAX = 524288) must be a multiple of 2
decimation                  = 2 #1216/2                   #Decimation
aquisiton_Amount            = 1000             #Nubmer of acquisitons
larmor_Frequency_Hertz      = 13400000         #Larmor frequency # 24.3417e+6   
excitation_duration_seconds = 20e-6     #Excitation time
delay_repeat_useconds       = 0.1e+6
experiment_name_all_files = "Stepfreq"

repeat                      = 5     #Number of folders
Number_of_files             = 400
step_freq                   = 3e+3
step_P90                    = 5e-6
...
print(f"Total time :"+str(datetime.timedelta(seconds=temps_secondes*Number_of_files*repeat)))
...
for j in range(repeat):
    ################ FOLDER NAME IS MADE TO WORK WITH OPEN_BIN_FIND_Freq ##############
    nameLocalFolder = nmr.create_file_wdate("FindFreqAuto_"+str(Number_of_files)+"_"+str(step_freq)+"_"+str(larmor_Frequency_Hertz))
    ##################################################################################

    excitation_duration_seconds_loop = excitation_duration_seconds + j*step_P90
    ...
    for i in range(Number_of_files):
        ...
        nmr.run_acquisition_command(sample_Amount, 
                                    ...
                                    excitation_duration_seconds_loop,
...
max_freq_amp = voltageAcc_array[np.argmax(tf_sum)]/np.min(voltageAcc_array)
print(f"\033[92m Larmor Frequency : {max_freq} Hz {max_freq_amp}\033[0m")
